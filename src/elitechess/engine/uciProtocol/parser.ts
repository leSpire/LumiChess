import type { ScoreModel } from "../../core/types.js";
import type { UciParsedMessage } from "./types.js";

const INFO_RE = /\b([a-z]+)\b/g;

function parseScore(tokens: string[]): ScoreModel {
  const scoreIndex = tokens.indexOf("score");
  const turn = "w";
  if (scoreIndex < 0) return { kind: "cp", value: 0, pov: turn };
  const kind = tokens[scoreIndex + 1];
  const rawValue = Number(tokens[scoreIndex + 2]);
  if (kind === "mate") return { kind: "mate", value: rawValue, pov: turn };
  return { kind: "cp", value: rawValue, pov: turn };
}

export function parseUciMessage(raw: string): UciParsedMessage {
  const line = raw.trim();
