...
import { NoopRealtimeSync } from "./network/realtimeSync/RealtimeSync.js";
...
export class LeLiteChessBoard {
    root;
    statusEl;
    sync;
    state = new GameStateEngine();
    highlights = new HighlightsManager();
    renderer;
    dragDrop;
    selected = null;
    constructor(root, statusEl, sync = new NoopRealtimeSync()) {
        this.root = root;
    ...
    statusLabel(snapshot) {
        ...
        if (snapshot.checkSquare && this.state.legalMoves().length === 0)
            return `Échec et mat · ${turn}`;
        if (this.state.legalMoves().length === 0)
            return "Pat";
        if (snapshot.checkSquare)
            return `Échec · ${turn}`;
